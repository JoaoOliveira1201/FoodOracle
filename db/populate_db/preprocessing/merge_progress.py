...
def merge_progress_files():
    ...
    print("Ñ Merging progress files...")
    ...
    print(f"ä Total locations: {total_generated}")
    print(f"ä User locations: {len(user_locations)}")
    print(f"ä City locations: {len(city_locations)}")

    # Clean up progress files
    print("\nüßπ Cleaning up progress files...")
    ...
    for file in progress_files:
        try:
            ...
            os.remove(file)
            print(f"ëÔ∏è Removed: {file}")
        except Exception as e:
            print(f"‚ö†Ô∏è Failed to remove {file}: {e}")

    print(f"\nâ Successfully merged {total_generated} locations!")
    print(f"Ñ Final file: {output_file}")
